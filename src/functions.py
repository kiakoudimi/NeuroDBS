...
import numpy as np
import pandas as pd
import io, msoffcrypto, os, re, random, nilearn.masking
...
def get_data(root, mask_img, ids=None):
    ...
    for path, subdirs, files in os.walk(root):
        for name in files:
            if fnmatch(name, pattern_on):
                on_state_paths.append(os.path.join(path, name))
            if fnmatch(name, pattern_off):
                off_state_paths.append(os.path.join(path, name))
    ...
    on_paths = sorted(on_state_paths, key=lambda path: int(re.search(r'(\d+)', path.split("/")[-1]).group()) if re.search(r'(\d+)', path.split("/")[-1]) else 0)
    off_paths = sorted(off_state_paths, key=lambda path: int(re.search(r'(\d+)', path.split("/")[-1]).group()) if re.search(r'(\d+)', path.split("/")[-1]) else 0)

    # If 'ids' is provided, filter the paths by subject ID
    if ids is not None:
        subject_numbers = [int(os.path.basename(path).split('_')[0]) for path in on_paths]
        on_paths = [path for path, num in zip(on_paths, subject_numbers) if f'SUBJECT_{num:03d}' in ids['Subject'].values]
        off_paths = [path for path, num in zip(off_paths, subject_numbers) if f'SUBJECT_{num:03d}' in ids['Subject'].values]
